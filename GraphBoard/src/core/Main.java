...
public class Main {
	...
	public static double u = 0.05;
	public static double t;
    public static SmartBoard sm;
	...
	public static void main(String[] args) throws InterruptedException {
		...
		home.pack();
        Y_BOUND = sm.getTree().getHeight();
        X_BOUND = sm.getTree().getWidth();
        randInit(0, 0, 0);
	...
	public static void randInit(int nodeN, int fixedN, int linkN){
		for(int i=0; i<nodeN; i++){
			nodes.add(new Node(Math.random()*X_BOUND, Math.random()*Y_BOUND, 300*Math.random() + 5, 10*(Math.random()-.5)));
		}
		for(int i=0; i<fixedN; i++){
			nodes.add(new FixedNode(Math.random()*X_BOUND, Math.random()*Y_BOUND, 300*Math.random() + 5, 10*(Math.random()-.5)));
		}
	...
	public static void init(){
		...
		nodes.add(new Node(265, 265, 250, 0));
		nodes.add(new Node(215, 265, 250, 0));
		...
		*/
		//links.add(new FixedLink(nodes.get(0), nodes.get(1), 50));
...
//					n1.setVelocityY(Vy1p);
	...
	public static void applyFriction(){
		for(Node n: nodes){
			n.applyForce(9.8 * u * n.getMass() * -1 * (n.getVelocityX() == 0? 0 
					: (n.getVelocityX()/Math.abs(n.getVelocityX()))),
					9.8 * u * n.getMass() * -1 * (n.getVelocityY() == 0? 0 
							: (n.getVelocityY()/Math.abs(n.getVelocityY()))));
		}
	}
